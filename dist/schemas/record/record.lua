...
local ____index = require("schemas.string.index")
local ____string = ____index.string
local ____values = require("schemas.record.values")
...
function ____exports.record(arg1, arg2, arg3, arg4)
    ...
    return {
        ...
        _parse = function(self, input, info)
            if not input or type(input) ~= "table" or tostring(input) ~= "[object Object]" then
                return {issues = {getIssue(info, {reason = "type", validation = "record", message = ____error or "Invalid type", input = input})}}
            end
            local issues
            local output = {}
