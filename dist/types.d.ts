...
import type { ArrayPathItem, MapPathItem, ObjectPathItem, RecordPathItem, SetPathItem, TuplePathItem } from './schemas/index';
/**
 * Parse info type.
 */
export type ParseInfo = Partial<Pick<Issue, 'origin' | 'path' | 'abortEarly' | 'abortPipeEarly'>>;
/**
 * Validate info type.
 */
 ...
 */
export type Input<TSchema extends BaseSchema | BaseSchemaAsync> = NonNullable<TSchema['types']>['input'];
/**
 * Output inference type.
 */
export type Output<TSchema extends BaseSchema | BaseSchemaAsync> = NonNullable<TSchema['types']>['output'];
/**
 * Validation and transformation pipe type.
 */
export type Pipe<TValue> = ((value: TValue, info: ValidateInfo) => ParseResult<TValue>)[];
/**
 * Async validation and transformation pipe type.
 */
export type PipeAsync<TValue> = ((value: TValue, info: ValidateInfo) => ParseResult<TValue> | Promise<ParseResult<TValue>>)[];
/**
 * Resolve type.
 *
